...
int main(int argc, char **argv) {
    ...
    int n = 0, p = 0, haveinit = 0, itmax = 0, feps = 0, ceps = 0, verbose = 0,
        width = 0, precision = 0, relax = 0, degree = 0, ordinal = 0,
        weights = 0, haveknots = 0, anchor = 0, ninner = 0, percentiles = 0,
        ditmax = 0, deps = 0, dverbose = 0, writefile = 0,
    ...
    (void)smacofReadParameterFile(
        parameterfile, &n, &p, &itmax, &haveinit, &feps, &ceps, &width,
        &precision, &verbose, &relax, &ditmax, &deps, &dverbose, &degree,
        &ordinal, &weights, &haveknots, &lowend, &highend, &anchor, &ninner,
        &percentiles, &writefile, &makeplots, &transform);
    ...
    if (haveinit == HAVE_INIT_CONFIGURATION) {
    ...
    if (ordinal) {
        (void)smacofCumsumMatrix(n, ncol, basis);
    }
    ...
    (void)smacofSSEngine(n, p, delta, w, vmat, vinv, xold, xnew, dmat, dhat,
                         basis, haveinit, itmax, feps, ceps, verbose, relax,
                         ditmax, deps, dverbose, weights, degree, ordinal,

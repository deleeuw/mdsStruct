...
void smacofPrintStrictLowerTriangularMatrix(const int n, const int width,
                                const int precision, double **x) {
    for (int i = 0; i < (n - 1); i++) {
        for (int j = 0; j <= i; j++) {
            printf("%+*.*f", width, precision, x[i][j]);
...
void smacofUnweightedWriteOutputFile(FILE *stream, const int n, const int p,
    ...
    fprintf(stream, "Transformed Data\n\n");
    smacofFPrintSymmetricMatrix(stream, n, width, precision, dhat);
